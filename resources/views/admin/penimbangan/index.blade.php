...
<div class="space-y-5">
    <div class="border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <a
                href="{{ route('admin.penimbangan.create') }}"
        ...
        <form method="GET" class="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <div class="flex gap-2">
                ...
                @if (request()->filled('search'))
                    <a
                        href="{{ route('admin.penimbangan.index') }}"
                        ...
                        <tr class="transition hover:bg-slate-50">
                            ...
                            <td class="px-4 py-4 text-sm whitespace-nowrap">
                                <div class="flex flex-nowrap items-center gap-2">
                                    <a href="{{ route('admin.penimbangan.show', $penimbangan) }}" class="inline-flex min-w-[72px] items-center justify-center border border-sky-600 px-3 py-2 text-xs font-medium text-sky-600 transition hover:bg-sky-600 hover:text-white">
                                        Detail
                                    </a>
                                    <a href="{{ route('admin.penimbangan.edit', $penimbangan) }}" class="inline-flex min-w-[72px] items-center justify-center border border-amber-500 px-3 py-2 text-xs font-medium text-amber-600 transition hover:bg-amber-500 hover:text-white">
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.penimbangan.destroy', $penimbangan) }}" method="POST" class="delete-penimbangan-form inline-flex" data-nama="{{ $penimbangan->bayi?->nama ?? 'data ini' }}">
                                        @csrf
                                        @method('DELETE')
