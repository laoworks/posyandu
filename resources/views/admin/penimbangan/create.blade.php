...
<div class="space-y-5">
    ...
    <form action="{{ route('admin.penimbangan.store') }}" method="POST" class="border border-slate-200 bg-white p-5 shadow-sm">
        @csrf
        ...
        @if ($balitas->isEmpty())
            <div class="mb-5 border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                ...
                <a href="{{ route('admin.balita.index') }}" class="font-medium text-[#3f37c9] underline">
                    Data Balita
                </a>.
            </div>
        @endif
