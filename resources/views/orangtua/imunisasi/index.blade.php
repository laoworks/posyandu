...
@section('page-description', 'Lihat riwayat imunisasi untuk anak yang terhubung ke akun ini.')

@section('content')
<div class="space-y-5">
    <div class="border border-slate-200 bg-white p-5 shadow-sm">
        <div>
            <h3 class="text-xl font-semibold text-slate-800">Riwayat Imunisasi Anak</h3>
            <p class="mt-1 text-sm text-slate-500">Riwayat imunisasi yang sudah tercatat dapat dipantau dari halaman ini.</p>
        </div>
    </div>
        ...
        <form method="GET" class="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <div class="flex gap-2">
                ...
                @if (request()->filled('search'))
                    <a href="{{ route('orangtua.imunisasi.index') }}" class="border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-600 transition hover:border-slate-400 hover:text-slate-800">
                        Reset
                    </a>
                @endif
            </div>
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse ($imunisasis as $item)
                        <tr class="transition hover:bg-slate-50">
                            ...
                            <td class="px-4 py-4 text-sm text-slate-600">{{ $item->jenis_imunisasi }}</td>
                            <td class="px-4 py-4 text-sm text-slate-600">{{ $item->dosis ?: '-' }}</td>
                            <td class="px-4 py-4 text-sm">
                                <a href="{{ route('orangtua.imunisasi.show', $item) }}" class="inline-flex min-w-[80px] items-center justify-center border border-sky-600 px-3 py-2 text-xs font-medium text-sky-600 transition hover:bg-sky-600 hover:text-white">
                                    Detail
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-10 text-center text-sm text-slate-500">
                                Belum ada riwayat imunisasi untuk anak Anda.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
