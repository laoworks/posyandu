...
@section('page-description', 'Lihat data anak yang terhubung dengan akun orang tua ini.')

@section('content')
<div class="space-y-5">
    <div class="border border-slate-200 bg-white p-5 shadow-sm">
        <div>
            <h3 class="text-xl font-semibold text-slate-800">Daftar Data Anak</h3>
            <p class="mt-1 text-sm text-slate-500">Profil anak yang sudah terhubung ke akun Anda ditampilkan di halaman ini.</p>
        </div>
    </div>
        ...
        <form method="GET" class="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <div class="flex gap-2">
                ...
                @if (request()->filled('search'))
                    <a
                        href="{{ route('orangtua.anak.index') }}"
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse ($anak as $item)
                        <tr class="transition hover:bg-slate-50">
                            <td class="px-4 py-4 text-sm text-slate-500">{{ $anak->firstItem() + $loop->index }}</td>
                            <td class="px-4 py-4">
                                <img
                                    src="{{ $item->foto ? asset('storage/'.$item->foto) : 'https://ui-avatars.com/api/?name='.urlencode($item->nama).'&background=EEF2FF&color=3f37c9' }}"
                                    alt="{{ $item->nama }}"
                                    class="h-11 w-11 object-cover"
                                >
                            </td>
                            <td class="px-4 py-4 text-sm text-slate-600 break-words">{{ $item->nik }}</td>
                            <td class="px-4 py-4 text-sm font-medium text-slate-800 break-words">{{ $item->nama }}</td>
                            <td class="px-4 py-4 text-sm text-slate-600">{{ $item->jenis_kelamin }}</td>
                            <td class="px-4 py-4 text-sm text-slate-600">{{ optional($item->tanggal_lahir)->format('d M Y') }}</td>
                            <td class="px-4 py-4 text-sm">
                                <a
                                    href="{{ route('orangtua.anak.show', $item) }}"
                                    class="inline-flex min-w-[80px] items-center justify-center border border-sky-600 px-3 py-2 text-xs font-medium text-sky-600 transition hover:bg-sky-600 hover:text-white"
                                >
                                    Detail
                                </a>
                            </td>
