...
@section('page-description', 'Lihat riwayat penimbangan untuk anak yang terhubung ke akun ini.')

@section('content')
<div class="space-y-5">
    <div class="border border-slate-200 bg-white p-5 shadow-sm">
        <div>
            <h3 class="text-xl font-semibold text-slate-800">Riwayat Penimbangan Anak</h3>
            <p class="mt-1 text-sm text-slate-500">Hasil penimbangan yang tercatat bisa dipantau kembali di halaman ini.</p>
        </div>
    </div>
        ...
        <form method="GET" class="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <div class="flex gap-2">
                ...
                @if (request()->filled('search'))
                    <a href="{{ route('orangtua.penimbangan.index') }}" class="border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-600 transition hover:border-slate-400 hover:text-slate-800">
                        Reset
                    </a>
                @endif
            </div>
            ...
            <table class="min-w-[980px] w-full table-fixed border-collapse">
                <thead class="bg-slate-50">
                    <tr class="border-b border-slate-200">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Nama Anak</th>
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse ($penimbangans as $item)
                        <tr class="transition hover:bg-slate-50">
                            <td class="px-4 py-4 text-sm text-slate-500">{{ $penimbangans->firstItem() + $loop->index }}</td>
                            <td class="px-4 py-4 text-sm font-medium text-slate-800">{{ $item->bayi?->nama ?? '-' }}</td>
                            <td class="px-4 py-4 text-sm text-slate-600">{{ optional($item->tanggal)->format('d M Y') }}</td>
                            <td class="px-4 py-4 text-sm text-slate-600">{{ number_format((float) $item->berat_badan, 2) }} Kg</td>
                            <td class="px-4 py-4 text-sm text-slate-600">{{ number_format((float) $item->tinggi_badan, 2) }} Cm</td>
                            <td class="px-4 py-4 text-sm">
                                <a href="{{ route('orangtua.penimbangan.show', $item) }}" class="inline-flex min-w-[80px] items-center justify-center border border-sky-600 px-3 py-2 text-xs font-medium text-sky-600 transition hover:bg-sky-600 hover:text-white">
                                    Detail
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-10 text-center text-sm text-slate-500">
                                Belum ada riwayat penimbangan untuk anak Anda.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
