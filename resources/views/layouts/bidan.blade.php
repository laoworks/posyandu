...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>@yield('title', 'Panel Bidan Desa') - {{ config('app.name', 'Laravel') }}</title>
    ...
    <body x-data="{ sidebarOpen: false }" class="h-screen overflow-hidden font-sans antialiased">
        <div class="h-screen bg-[#f8f9fc]">
            <div class="flex h-full">
                ...
                @include('layouts.partials.bidan-sidebar')
                    ...
                    <header class="border-b border-slate-200 bg-white px-6 py-4 sm:px-8">
                        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                <p class="text-xs font-semibold uppercase tracking-[0.22em] text-[#3f37c9]">Halaman Bidan Desa</p>
                                <h2 class="mt-1 text-[28px] font-semibold text-slate-800">@yield('page-title', 'Dashboard Bidan Desa')</h2>
                                <p class="mt-1 text-sm text-slate-500">@yield('page-description', 'Kelola pelayanan pemeriksaan dan pemantauan kesehatan balita.')</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-3">
                                <div class="hidden text-right sm:block">
                                    ...
                                    <p class="mt-1 text-sm font-medium text-slate-700">{{ auth()->user()->name ?? 'Bidan Desa' }}</p>
                                </div>
